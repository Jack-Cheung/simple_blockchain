...
read_only::get_table_rows_result read_only::get_table_rows( const read_only::get_table_rows_params& p )const {
   ...
   std::cout << "----> p.code=" << p.code << " p.scope=" << p.scope << " p.json=" << p.json
      << " p.table=" << p.table << " table_key=" << p.table_key << " p.lower_bound=" << p.lower_bound << " p.upper_bound="
      << p.upper_bound << " limits=" << p.limit << std::endl;
   bool primary = false;
...
void read_write::push_transactions(const read_write::push_transactions_params& params, next_function<read_write::push_transactions_results> next) {
   ...
   {
      std::cout << "#" ;
      fc::json::to_stream(std::cout, param);
      std::cout << std::endl;
   }
   try {
